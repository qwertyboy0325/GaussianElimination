	...
	for (int i = min_size; i <= max_size; i++) {
		...
		for (size_t threads_num = 1; threads_num <= max_thread; threads_num++) {
			...
			if (numValidTests > 0) {
				...
				result.threadExecutionTimes.push_back({ threads_num, averageExecutionTime });
			}
		}
...
void exportToCSV(const std::vector<PerformanceResult>& results) {
	...
	std::filesystem::path filename = outputPath / ("performance_results_" + ss.str() + ".csv");
	...
	for (const auto& result : results) {
		file << result.matrixSize << "," << result.threadExecutionTimes.size() << ",";
		for (size_t i = 0; i < result.threadExecutionTimes.size(); i++) {
			file << result.threadExecutionTimes[i].second;
			if (i < result.threadExecutionTimes.size() - 1)
				file << ",";
		}
		file << std::endl;
	}
